...
_PREREQ_EXTRACTION_DIRECTIVE = (
    ...
    "  - Post-install artifacts: [EVERY resource, object, config file, or "
    "instance that must be created AFTER install for the component to "
    "function — e.g. custom resource instances, config maps, service "
    "accounts, init commands, database schemas. Installing a tool without "
    "configuring it is useless.]\n\n"
    ...
    "CRITICAL — installing software is NEVER enough. Most platforms require "
    "you to create instances, resources, or configuration AFTER install. "
    "If your docs mention 'create an instance', 'apply a CR', 'initialize', "
    "'configure', or 'create a resource' — that step is MANDATORY. "
    "Skipping it means the software is installed but does nothing.\n\n"
...
_RESEARCH_SUMMARY_DIRECTIVE = (
    ...
    "**Post-install artifacts per component**: [for EACH component, list every "
    "resource/instance/config that must be created AFTER install — extract these "
    "from the docs you read, not from memory. Include the exact resource type, "
    "name, namespace/location, and any critical field values like source "
    "repositories, channels, or version pins.]\n"
    ...
    "- For each component, did you extract the EXACT post-install steps from "
    "the docs (not from your training data)?\n"
    "- If the docs page you fetched returned irrelevant content, did you retry "
    "with a different URL or search query?\n"
