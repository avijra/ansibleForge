...
_COMPACTION_PROMPT = (
    ...
    "Summarize the conversation into the EXACT sections below. "
    "Omit tool call IDs, reasoning tokens, and verbose output.\n\n"
    "## GOAL\n"
    "One sentence: what the user originally asked for.\n\n"
    "## CRITICAL PATHS\n"
    "List EVERY absolute path referenced (kubeconfig, inventory, SSH key, "
    "certificates, config files). Preserve verbatim — never shorten or relativize.\n\n"
    "## FILES GENERATED\n"
    "List ALL files created/modified under the workspace, grouped by type:\n"
    "- Roles: roles/<name>/ (purpose)\n"
    "- Playbooks: playbooks/<name>.yml (which roles)\n"
    "- Inventory: inventory/<file> (hosts/groups)\n"
    "- Terraform: terraform/<file> (resources)\n"
    "- Other: templates, configs, scripts\n\n"
    "## DECISIONS\n"
    "Key choices made and WHY (region, instance type, catalog source, version). "
    "One bullet per decision.\n\n"
    "## INFRASTRUCTURE STATE\n"
    "Resources created/modified/destroyed with IDs, IPs, endpoints. "
    "Include cluster names, namespaces, operator versions.\n\n"
    "## SECRETS COLLECTED\n"
    "List EVERY secret name stored (agent must NOT re-request these).\n\n"
    "## COMPLETED STEPS\n"
    "Numbered list of steps finished, with verification status if known "
    "(e.g., '3. Deployed GPU Operator — VERIFIED: driver pods running').\n\n"
    "## PENDING STEPS\n"
    "What remains to be done, in order.\n\n"
    "## ERRORS AND FIXES\n"
    "Errors encountered and how they were resolved. One bullet per error.\n\n"
    "## RESEARCH FINDINGS\n"
    "Key version numbers, prerequisites, dependency chains, and compatibility "
    "constraints discovered from documentation.\n\n"
    "Keep each section to 3-8 bullets maximum. Total summary under 60 lines."
)
